...
> 3. 判断是否已经全部加载完毕，停止监听
>
> `v-infinite-scroll` 是 `element-plus` 默认自带的指令。https://github.com/ElemeFE/vue-infinite-scroll
>
> 在 `element-plus` 的文档中，为 `v-infinite-scroll` 指令明确提供了 `infinite-scroll-disabled` 这个选项。当它的值为 `true` 时，可以用于禁用无限滚动的功能。
