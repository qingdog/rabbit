...
`useIntersectionObserver`监听一个元素与其祖先或视口的交叉情况（即元素是否在用户的视野内）

插件的 `install` 方法，它会在插件安装时被调用。`install` 方法接收一个参数 `app`，它是一个 Vue.js 的应用实例。

在 `install` 方法中，你可以执行各种操作，比如注册全局组件、指令、混入等，也可以在应用实例中注入一些全局方法或属性。
